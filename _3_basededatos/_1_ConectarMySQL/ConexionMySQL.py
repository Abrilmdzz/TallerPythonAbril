...
class ConexionMysql:
    ...
    def Insertar(self, tabla, valores)->bool:
        sql = f"INSERT INTO {tabla} (id_rol, nombre_completo,  nombre_usuario, correo_electronico, contrasennia ) VALUES (%s, %s, %s)"
...
if __name__ == '__main__':
    ...
    if conexion.BanConexion:
        ...
        res = conexion.Consultar('select * from roles')
        if res != None:
            for fila in res:
                print(
                    f"ID: {fila[0]}, Titulo del Rol: {fila[1]}")
